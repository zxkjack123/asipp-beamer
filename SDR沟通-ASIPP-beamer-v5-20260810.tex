...
\begin{frame}{反馈意见概述}
	\begin{block}{原反馈（第3条）}
		\textbf{建议增加停机剂量率（Shutdown Dose Rate, SDR）的明确计算要求。}
	\end{block}
	\vspace{4pt}
...
\begin{frame}{具体建议}
	\begin{enumerate}
		\item[(a)] 人员可进入的维护区域在不同冷却时间下的 SDR 空间分布
...
\begin{frame}{SDR 在聚变安全分析中的不可替代性}
	\begin{columns}[T]
		...
		\textbf{运行剂量率 vs 停机剂量率}
		\begin{itemize}
			\item 瞬发剂量率（运行）→ 屏蔽设计
			\item \textbf{SDR（停机后）→ 维修可达性}
...
\begin{frame}{ITER 第一壁 SDR 演化}
	...
	\begin{tikzpicture}[scale=0.70]
		% Axes
		\draw[->] (0,0) -- (8.5,0) node[right] {\small 冷却时间};
		\draw[->] (0,0) -- (0,4.8) node[above] {\small 接触剂量率 (Sv/h)};
		% Log-scale y-axis labels
		\foreach \y/\ylbl in {0/10^{-4}, 1/10^{-2}, 2/10^{0}, 3/10^{2}, 4/10^{4}} {
		\draw (-0.15,\y) -- (0.15,\y) node[left] {\tiny $\ylbl$};
		}
		% x-axis labels
		\foreach \x/\xlbl in {0/停机, 2/1天, 4/1周, 5.5/1月, 7/1年, 8/10年} {
				\draw (\x,0) -- (\x,-0.15) node[below] {\tiny \xlbl};
			}
		% Data curve
		\draw[thick, blue!60!black] (0,4.0) -- (1,3.2) -- (2,2.0) -- (4,1.7) -- (5.5,1.3) -- (7,0.7) -- (8,0.0);
		% Data points
		\fill[red] (0,4.0) circle (1.5pt) node[above right] {\tiny $\sim10^4$};
		\fill[red] (2,2.0) circle (1.5pt) node[above right] {\tiny $\sim10^2$};
		\fill[red] (4,1.7) circle (1.5pt) node[above] {\tiny $\sim50$};
		\fill[red] (5.5,1.3) circle (1.5pt) node[above right] {\tiny $\sim20$};
		\fill[red] (7,0.7) circle (1.5pt) node[above] {\tiny $\sim5$};
		\fill[red] (8,0.0) circle (1.5pt) node[above right] {\tiny $\sim1$};
		% Nuclide annotations
		\node[font=\tiny\itshape, text=green!50!black] at (1,4.2) {$^{56}$Mn};
		...
		% Title
		\node[font=\footnotesize] at (4.2,5.0) {SS-316LN 第一壁活化衰变曲线（参考 ITER 典型值）};
	\end{tikzpicture}
	...
	\begin{table}
		...
		\begin{tabular}{c|c|c|c}
			\toprule
			\textbf{冷却} & \textbf{接触剂量率}    & \textbf{1m处}      & \textbf{主导核素}        \\
			\midrule
			停机即时        & $\sim10^{4}$ Sv/h & $\sim10^{2}$ Sv/h & $^{56}$Mn            \\
...
\begin{frame}{拟新增条文（建议 §10.2 或 §5.1）}
	\begin{block}{建议条文}
		应评估装置停机后人员可能进入维护区域的残余剂量率（停机剂量率，SDR），给出维护区域在不同冷却时间下的 SDR 空间分布。冷却时间点应根据装置运行方案和维护计划合理设定。说明所采用的 SDR 计算方法（如严格两步法 R2S 或直接输运法），并将结果与维修人员剂量约束值进行对比分析。
	\end{block}
